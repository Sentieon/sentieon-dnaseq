...
# Log file
logfile=$PWD/Sentieon_match3.8_4.0_$(date +"%Y%m%d%H%M").log
exec >$logfile 2>&1
...
mkdir -p sentieon_match3.8_4.0
cd sentieon_match3.8_4.0
output_prefix=${prefix}sentieon_match3.8_4.0_
...
command="$release/bin/sentieon driver -r $fasta -t $cpu_cores --interval $calling_intervals_list --algo GVCFtyper --emit_conf 10 --call_conf 10 -d $known_dbsnp -v ${output_prefix}output.g.vcf.gz ${output_prefix}output.vcf.gz"
